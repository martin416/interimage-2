...
/**
 * A space filling curve tile manager implementation.
 * @author Rodrigo Ferreira
 *
 */

public class SFCTileManager implements TileManager {
	...
	public SFCTileManager(double size, String crs) {
		setSize(size, crs);
	...
	public String encode(long id) {
		...
		return encodeCoordinates(i, j, 0);
	}
	
	public String encode(long id, int level) {
	...
	public List<String> getNeighourTiles(String code, List<String> directions) {
		...
		if (directions.contains("N")) {
			neighbours.add(root + getUpperLevelNeighbor(currentLevel, upperLevel, "N") + _neighbors.get("N")[_directionsMap.get(currentLevel)]);
		}
		
		//NE
		if (directions.contains("NE")) {
			neighbours.add(root + getUpperLevelNeighbor(currentLevel, upperLevel, "NE") + _neighbors.get("NE")[_directionsMap.get(currentLevel)]);
		}
		
		//E
		if (directions.contains("E")) {
			neighbours.add(root + getUpperLevelNeighbor(currentLevel, upperLevel, "E") + _neighbors.get("E")[_directionsMap.get(currentLevel)]);			
		}
		
		//SE
		if (directions.contains("SE")) {
			neighbours.add(root + getUpperLevelNeighbor(currentLevel, upperLevel, "SE") + _neighbors.get("SE")[_directionsMap.get(currentLevel)]);
		}
		
		//S
		if (directions.contains("S")) {
			neighbours.add(root + getUpperLevelNeighbor(currentLevel, upperLevel, "S") + _neighbors.get("S")[_directionsMap.get(currentLevel)]);
		}
		
		//SW
		if (directions.contains("SW")) {
			neighbours.add(root + getUpperLevelNeighbor(currentLevel, upperLevel, "SW") + _neighbors.get("SW")[_directionsMap.get(currentLevel)]);			
		}
		
		//W
		if (directions.contains("W")) {
			neighbours.add(root + getUpperLevelNeighbor(currentLevel, upperLevel, "W") + _neighbors.get("W")[_directionsMap.get(currentLevel)]);
		}
		
		//NW
		if (directions.contains("NW")) {
			neighbours.add(root + getUpperLevelNeighbor(currentLevel, upperLevel, "NW") + _neighbors.get("NW")[_directionsMap.get(currentLevel)]);
		}
